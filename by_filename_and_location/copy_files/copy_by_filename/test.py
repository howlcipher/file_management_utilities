...
def copy_files(config, logger):
    ...
    file_copy_log_name = f"{current_date}_file_copy_log-test.txt"
        ...
        for root, dirs, files in os.walk(source_directory):
            for directory in dirs:
                ...
                for filename in os.listdir(directory_path):
                    ...
                    if not matched:
                        print(f"File does not match: {file_path}")  # Print each file not matched
                        file_copy_log.write(f"File does not match: {file_path}\n")  # Log each file not matched
                        continue

                    print(f"Found file: {file_path}")  # Print each file found
                    file_copy_log.write(f"Found file: {file_path}\n")  # Log each file found
...
def main():
    with open("config.json", "r") as f:
        ...
        config = Config(
            ...
            config_data.get("max_files", False),
            config_data.get("max_files_value", 10)
        )

    current_month_day = datetime.datetime.now().strftime("%m%d")
    log_file_name = f"{current_month_day}_log-test.txt"
